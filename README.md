...
- `create_office_file` 还可以继续用。想把 Word 做得更稳一点，走四步链会更合适。
- Word 导出链已经支持双后端：插件 runtime 默认优先走 JS 渲染器，失败会自动回退到 Python 渲染器。
...
1. 插件 runtime 优先走 JS 渲染器
2. 如果 JS 渲染失败，并且 `word_render_fallback_enabled=true`
3. 自动回退到 Python 渲染器

如果你在本地直接调用 `build_document_toolset()` 做测试，默认仍只走 Python，这样不会因为 JS 没构建而把现有测试拖慢。
