...
- `create_office_file` 还可以继续用；它的 Word 导出现在也走同一套后端调度。
- Word 导出链已经支持双后端：默认优先走 JS 渲染器，失败会自动回退到 Python 渲染器。
...
| `word_render_backend` | `node` | 想强制只走 Python 渲染器，或者调试 JS 渲染器 |
| `word_render_fallback_enabled` | `true` | 想让 Node 导出失败时直接报错，不自动回退 |
...
### Word 渲染（`render_settings`）

| 配置项 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| Word 渲染后端 (`word_render_backend`) | string | node | `node` 表示优先用 JS 渲染器；`python` 表示只走现有 Python 渲染器 |
| 启用 Python 回退 (`word_render_fallback_enabled`) | bool | true | 开启后，Node 渲染失败会自动回退到 Python |
| Node 渲染器入口 (`node_renderer_entry`) | string | 空 | 可选，自定义 Node CLI 路径；留空时默认找 `word_renderer_js/dist/cli.js` |
...
复杂 Word 四步链现在有两套渲染后端：

- JS 渲染器：位于 `word_renderer_js/`，由 Python 在导出阶段调用 `node dist/cli.js input.json output.docx`
- Python 渲染器：继续保留，主要负责回退和兼容

目前的默认策略是：

1. 默认入口优先走 JS 渲染器
2. 如果 JS 渲染失败，并且 `word_render_fallback_enabled=true`
3. 自动回退到 Python 渲染器

现在不只是插件 runtime 和 `build_document_toolset()`，更底层的默认后端选择也已经统一为“JS 优先，Python 回退”。

如果本地还没构建 `word_renderer_js/dist/cli.js`，导出时会直接回退到 Python 渲染器，不会阻塞现有功能。
