...
def build_document_tools_detail_notice() -> str:
    return (
        ...
        "[工具选择]\n"
        "- 复杂 Word 文档 → 使用上述工具链\n"
        '- PPT/演示文稿 → 使用 `create_document(format="ppt")` → `add_slides`（slide types）→ `export_document`\n'
        ...
        "- 正文表格单元格不要传 `row_span` 或 `col_span`；这组公共文档工具不提供单元格合并输入\n"
        "- 预设不够时再用 `style={align, emphasis, font_scale, table_grid, cell_align}`"
        " 和 `layout={spacing_before, spacing_after}`\n"
        "- `heading` 只使用 `text`，不要传 `title`；表格标题才放到 `table.caption` 或 `table.title`\n"
        "- run 级颜色字段使用 `color`，不要传 `text_color`；`text_color` 只用于 hero_banner 和 table cell\n"
        "- 正文表格单元格对象不要传 `type=cell`，直接传 `{text, runs, fill, text_color, border...}`\n"
        ...
        "- PPT 文档使用 `add_slides` 而非 `add_blocks` 添加内容\n"
        "- PPT 专用 slide 类型：\n"
